...
from data import generate_convex_combinations, generate_alpha_combinations, run_evolutionary_study_with_replicates_with_ph_match
...
def load_generated_data(signature):
    ...
    #my dilema is to find a way to load the data for this signature folder
    ...
    for selection_strength in selection_strengths:
        ...
        for i, (alpha_ph_rob, alpha_ph_match, alpha_n_attractors) in sorted(enumerate(all_alphas), key=lambda x: x[1][0]):
            data = run_evolutionary_study_with_replicates_with_ph_match(population_size, q, g, N, n=n, k=k, 
                                          ...
                                          indegree_distribution=indegree_distribution, n_simulations_per_analysis=n_sim, selection_method=selection_method, selection_strength=selection_strength, Debug=DEBUG_MODE, data_path=data_path)
...
if __name__ == "__main__":
    ...
    n_reps = 2
    alpha_dyn = [0, 0.005]
    convex_2d = generate_convex_combinations(1/n_alphas,dimension=2)
    ...
    signature = hash_params(M=M, q=q, g=g, N=N, n=n, k=k, mutation_probability=mutation_probability, selection_method=selection_method, selection_strengths=selection_strengths, indegree_distribution=indegree_distribution, n_alphas=n_alphas, population_size=population_size, n_reps=n_reps)
    ...
    data.to_csv(f'../master_data_file.csv', mode='a')
    data_path = os.path.join('../data/', f'{signature}')
